...
| ID | 遗留点 | 当前风险 | 下一步建议 |
|---|---|---|---|
| P0-70 | 日志与健康检查落盘 | 现有 `reset-demo`、验证脚本、`upgrade` 已落盘；`start`、`migrate`、`health` 仍主要输出终端 | 统一 `logs/ops/` 日志包装，补健康检查结果文件 |
| P0-71 | 迁移前备份与失败回滚演练 | `make migrate` 已备份，但失败回滚尚未演练 | 增加可控失败迁移脚本或 dry-run 验证 |
| P0-72 | 恢复点机制验证 | 已有路径恢复和损坏兜底代码，但未形成专项验证 | 增加 `make verify-update-restore` 或验证页入口 |
| P0-73 | 浏览器缓存清理验证 | 已验证 `version.json` / `/api/version` / `/` 禁缓存；旧静态资源错版未专项验证 | 增加构建 hash / 旧资源 404 / fallback 验证 |
